...
class Course(models.Model):
    ...
    def enroll_student(self, student):
        ...
        if not Class.objects.filter(course=self, students=student).exists():
            ...
            send_mail('[RiW] Usuário Cadastrou-se em um Curso', get_template('core/email/email_user_signed_up_course_support.txt').render(Context({'username': student.username, 'course_name': self.name, 'datetime': now.strftime("%d/%m/%Y - %H:%M"), 'email': student.email})), settings.EMAIL_SUPPORT, [settings.EMAIL_SUPPORT])
        if not CourseStudent.objects.filter(course=self, user=student).exists():
            if self.tuition == 0:
                ...
                send_mail('Inscrição no Curso Read in Web', get_template('core/email/email_user_signed_up_paid_course.txt').render(Context({'username': student.username, 'course_name': self.name, 'course_link': course_link})), settings.DEFAULT_FROM_EMAIL, [student.email])
