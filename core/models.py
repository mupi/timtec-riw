...
class Course(models.Model):
    ...
    def enroll_student(self, student):
        ...
        if not Class.objects.filter(course=self, students=student).exists():
            ...
            send_mail('[RiW-Timtec] Inscrição em curso', get_template('core/email/email_user_signed_up_course_support.txt').render(Context({'username': student.username, 'course_name': self.name, 'datetime': now.strftime("%d/%m/%Y - %H:%M"), 'email': student.email})), settings.DEFAULT_FROM_EMAIL, [settings.EMAIL_SUPPORT])
