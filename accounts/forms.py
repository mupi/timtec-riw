...
class SignupForm(AcceptTermsForm):
    ...
    def signup(self, request, user):
        ...
        user.save()
        send_mail('[RiW-Timtec] Usuário novo', get_template('account/email/email_new_user_message.txt').render(Context({'date': now.strftime("%d/%m/%Y"), 'time': now.strftime("%H:%M"), 'username': username, 'email': email})), settings.EMAIL_SUPPORT, [settings.EMAIL_SUPPORT])
